  ...
  return (
    <Link to={`/tenantprofile/${data._id}`}>
      ...
      <div className='lg:ml-10 ml-3 mt-8 flex flex-col gap-1 '>
        <p className='lg:text-xl font-bold text-base'> <span>{data.name}</span></p>
        <p className='text-sm font-semibold'>{data.phone}</p>
        <p className='bg-dark-blue text-white font-regular text-sm rounded-md p-2 w-36 mt-2 hover:opacity-70'>Balance: <span className='font-semibold'>{data.balance ? data.balance : 0}</span></p>
