...
const PetReadRow = ({ id, pets, handleEditClick, handleDeleteClick }) => {
  return (
    <tr className="text-center">
      <td className="px-4 py-2 pt-2">{pets.name}</td>
      <td className="px-4 py-2 pt-2">{pets.species}</td>
      <td className="px-4 py-2 pt-2">
        {new Date(pets.birthday).toLocaleDateString()}
      </td>
      <td className="px-4 py-2 pt-2">
        <button
          type="button"
        ...
        <button
          type="button"
          onClick={() => handleDeleteClick(id, pets._id)}
