  ...
  return (
    <div className=' overflow-hidden max-h-screen max-w-screen'>
      <div className='hidden lg:flex lg:items-center lg:justify-between w-screen px-20 pt-5'>
            ...
            <div>
              <button className='hidden lg:block bg-white text-black px-10 py-1 rounded-xl border-dark-blue border-2 hover:bg-light-blue hover:text-white hover:outline-none' onClick={toggleAction}>{action==='Log in' ? 'Sign Up' : 'Log in'}</button>
          </div> 
          </div>
         ...
         <div className=' flex flex-col w-screen h-screen items-center lg:flex-row justify-center'>
              ...
              <form className='flex flex-col gap-2 w-full px-20 lg:w-1/2' onSubmit={handleSubmit}>
                  <div className=' hidden lg:block lg:-mt-20 -mt-40'>
                    <h1 className='text-light-blue text-4xl font-medium'>{action}</h1>
                    <p className='lg:py-2 text-dark-gray text-lg '>{action==='Log in'?'Log in to your apartment rental management software.' : 'Create your account for accessing the apartment rental management platform.'}</p>
                  </div>
                  ...
                  <div>
                    <label className='relative cursor-pointer'>
                      ...
                      <span className='text-medium text-light-gray text-opacity-80  absolute left-1 bottom-5 px-1 '>Username</span>
                    </label>
                  ...
                  <button className='lg:mt-2 py-4 rounded-lg text-white bg-light-blue text-xl hover:opacity-80' type="submit">{action}</button>
                  {action === 'Log in' && (
                    <p className='hidden lg:block text-dark-gray my-2'>Create your account <span onClick={toggleAction} className='text-light-blue cursor-pointer'>here</span> .</p>
                  )}
