...
const cctvSlice = createSlice({
  ...
  reducers: {
    ...
    deleteCCTVSuccess: (state, action) => {
      state.loading = false
      state.data = state.data.map(item => item._id === action.payload ? {...item, ...action.payload} : item)
    },
...
export  const {startLoading, fetchCCTVSuccess, editCCTVSuccess, deleteCCTVSuccess, actionCCTVFailed} = cctvSlice.actions
...
export const deleteCCTV = (apartmentId, cctvId) => async(dispatch) => {
  try{
    dispatch(startLoading())
    const cctv = await fetch(`${cctv_url}/${apartmentId}/delete_cctv/${cctvId}`, {
      method: "DELETE",
      headers:{
        Authorization: `Bearer ${token}`
      },
