...
export const readNotification = (notif_id, navigate) => async (dispatch) => {
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_URL}/api/notification/update?notif_id=${notif_id}`,
      {
        method: 'PATCH',
        headers: {
...
export const fetchNotifications = () => async (dispatch) => {
  try {
    ...
    dispatch(fetchNotifStart())
    const response = await fetch(
      `${import.meta.env.VITE_URL}/api/notification/list`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const fetchNotification = () => async (dispatch) => {
  try {
    ...
    dispatch(fetchNotifStart())
    const response = await fetch(
      `${import.meta.env.VITE_URL}/api/notification/list/v1`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const deleteNotification = (notif_id) => async (dispatch) => {
  try {
    ...
    dispatch(fetchNotifStart())
    const response = await fetch(
      `${import.meta.env.VITE_URL}/api/notification/delete?notif_id=${notif_id}`,
      {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
