...
module.exports.create_apartment_unit = async (req, res) => {
    try{
        ...
        if(await UNITMODEL.findOne({unit_no:unit_no})){
            return res.status(httpStatusCodes.BAD_REQUEST).json({error: "Unit exists"})
        }
        let response = await UNITMODEL.create(details)
        if(!response){
            return res.status(httpStatusCodes.BAD_REQUEST).json({error: "Failed to create unit"})
        }
        const apartment_response = await APARTMENTMODEL.findById({_id:apartment_id})
        if(!apartment_response){
            return res.status(httpStatusCodes.BAD_REQUEST).json({error: "Unable to find apartment"})
        }
...
module.exports.edit_apartment_unit = async (req, res) => {
    try{
        ...
        return res.status(httpStatusCodes.OK).json({msg: `Updated ${details.unit_no}`})
    }
    catch(err){
        console.error({error: err.message})
        return res.status(httpStatusCodes.INTERNAL_SERVER_ERROR).json({error: "Server Error..."})
...
module.exports.delete_apartment_unit = async (req, res) => {
    try{
        ...
        const index = apartment_response.units.findIndex(item => item.toString() === unit_id)
        if(index === -1){
            return res.status(httpStatusCodes.BAD_REQUEST).json({error: "Unable to delete Unit"})
        }
