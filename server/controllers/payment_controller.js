...
module.exports.createIntent = async (req, res) => {
  try {
    ...
    const newAmountPaid =
      parseInt(invoice.payment.amountPaid || 0) + parseInt(amount)
...
module.exports.cashPayment = async (req, res) => {
  try {
    ...
    pdf.create(htmlContent, {
      ...
      if (err) {
        return res.status(httpStatusCodes.INTERNAL_SERVER_ERROR).json({ error: 'Failed to generate PDF' });
      }
      ...
      if (!cloudinaryResponse || !cloudinaryResponse.public_id) {
        return res.status(httpStatusCodes.CONFLICT).json({ error: 'Failed to upload PDF to Cloudinary...' });
      }

      const tenant = await TENANTMODEL.findByIdAndUpdate(response.tenant_id._id, { $inc: { balance: -response.payment.amountPaid } }, { new: true });

      if (!tenant) {
        return res.status(httpStatusCodes.BAD_REQUEST).json({ error: 'Unable to update tenant balance...' });
      }
      ...
      const mailOptions = {
        ...
        attachments: [{ filename: 'invoice.pdf', content: await downloadPdfFromCloudinary() }],
      };
      ...
      const admin = await USERMODEL.findOne({ role: 'Admin' });
      if (!admin) {
        return res.status(httpStatusCodes.BAD_REQUEST).json({ error: 'Invoice cannot be updated.' });
      }
      ...
      if (!sendNotif) {
        return res.status(httpStatusCodes.BAD_REQUEST).json({ error: 'Cannot send Notification' });
      }

      return res.status(httpStatusCodes.OK).json({ msg: 'Payment has been made.', response });
    });
  } catch (err) {
